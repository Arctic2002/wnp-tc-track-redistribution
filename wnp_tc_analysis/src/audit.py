...
def initial_registry() -> pd.DataFrame:
    ...
    for r in selected.itertuples():
        rows.append(dict(
            ...
            source_output=str(trend_path.relative_to(WORK)), status="verified", notes=f"FDR family: {r.fdr_family}",
        ))
    ...
    for r in pd.read_csv(red_path).itertuples():
        rows.append(dict(
            ...
            status="verified", notes="No FDR applied to pre-specified field-level permutation test",
        ))
    ...
    for r in pd.read_csv(agreement_path).itertuples():
        if r.metric in {"path_change_map", "annual_lmi_lat_common"}:
            rows.append(dict(
                ...
                status="verified", notes="Agency records share observational basis and are not independent experiments",
            ))
    ...
    for r in pd.read_csv(unique_path).itertuples():
        rows.append(dict(
            ...
            source_output=str(unique_path.relative_to(WORK)), status="verified",
            notes="Raster-densified crossing input; direct landfall latitude uses the separate exact-intersection analysis",
        ))
...
def run() -> None:
    ...
    inventory_paths = [
        ...
        (WORK / "MD/Manuscript_CN.md", "Chinese manuscript"),
        (WORK / "MD/Manuscript_EN.md", "English manuscript"),
        (WORK / "DOCX/Manuscript_CN.docx", "Chinese DOCX"),
    ]
...
- 中文、英文及补充材料文本位于`MD/`，对应Word文档位于`DOCX/`。
- 论文题名、作者、目标期刊和引用信息记录于公开包的 `README.md` 与 `CITATION.cff`。

## 2. 数据清点

- 最佳路径：IBTrACS WP v04r01，输入位于`data/raw/IBTrACS.WP.v04r01.csv`，标准化表和元数据位于`data/raw/ibtracs_wp.csv`和`ibtracs_wp.metadata.json`。
...
- ENSO/PDO：`data/raw/indices/oni.csv`和`pdo.csv`；来源记录见`metadata/DATA_SOURCES.md`。
...
- 主图文件{figure_counts['main']}个，补充图文件{figure_counts['supplementary']}个。
- 分析使用固定随机种子202406，参数由`config/analysis.yml`统一定义。
- `results_registry.csv`登记频数、LMI、路径场、机构一致性和唯一风暴登陆结果。

## 5. 初步一致性结论

- 文稿中的三机构频数斜率、路径场TVD/p值、路径空间相关和唯一风暴登陆份额变化均能在现有CSV中定位。
- 直接登陆纬度基于轨迹线段与高分辨率海岸线的几何交点，并使用互斥岸段归属。
- 固定588 dagpm是背景定位量，不解释为副高动力增强。
...
P0直接登陆纬度与三机构核心结果使用预设口径复算。P1分析不以显著性筛选替换预设主分析。
"""
