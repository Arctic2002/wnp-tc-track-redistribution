...
本目录对`../results/`中的CSV进行可追溯汇总，不重新计算OHC场、路径匹配、分解或bootstrap。
...
该入口在绘图前核对南北权重闭合，并核对分区加总与主配置`P`、`O`分量一致；同时输出中英文PNG/PDF及可追溯的Flint语义规格。20°N只作为描述性加总界线，图中不提供未计算的分区级bootstrap区间。
