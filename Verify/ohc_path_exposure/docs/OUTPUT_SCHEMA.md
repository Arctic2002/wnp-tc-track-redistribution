...
| `track_extraction_qc.csv` | `agency, definition, year, storms, points` | 机构覆盖记录 |
...
| `match_qc.csv` | `agency, definition, track_season, calendar_year, total, matched, outside_domain, outside_ohc_time_window, too_far, missing_ohc_file, matched_land_points, matched_ocean_points` | 匹配率、跨年时次、陆上吸附与距离门限记录 |
| `decomposition_summary.csv` | 分期、机构、定义、年份口径、海陆口径、权重、`exposure_p1/p2, delta_exposure, redistribution_component, ocean_component, covariability_component, closure_residual` | 结果数值的唯一摘要源 |
...
| `product_seam_diagnostics.csv`及`qa/product_seam_audit.json` | 2014/2015 结构、坐标容差/实际最大差值、有效足迹和数值诊断 | 产品拼接判读依据 |
